...
Теперь, если вы внесли в какой-то файл или файлы изменения, то необходимо также кликнуть правой кнопкой в любом месте папки и нажать *Git commit > master*. Ввести краткое описание изменений и нажать *Commit*, а затем *ПКМ* > *Tortoise Git* > *Push*. Локальные изменения попадут в ваш репозиторий на GitHub.

Чтобы добавить новый файл в репозиторий, нужно кликнуть по нему или по пустому месту в репозитории и выбрать команду *Add* (или *TortoiseGit > Add* ). Затем также выполнить *Push* для загрузки файлов в удаленный репозиторий.
